...
之前我们写二维数组的需要传参给一个函数的时候，我们是这样子写的


```c
void test(int arr[5][5], int row, int col)
{
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
...
![](/images/c/ef7159fb41c4462d9ec3f3a0938a73fe.png)

所以二维数组的数组明表示的就是第一行的地址，是一维数组的地址

上面所写的代码第一行的一维数组类型就是int[5]，所以第一行的地址的类型就是数组指针类型

int（\*）[5]，意味着二维数组传参本质上也是传递了地址，传递的是第一行这个一维数组的地址
...
把函数的地址存放到一个数组里面，那么这个数组就叫函数指针数组
